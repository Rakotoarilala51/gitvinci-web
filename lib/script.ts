...
export function generateBashScript(
  ...
  lines.push("# Recreates a pixel-art pattern on your GitHub contribution graph.");
  lines.push("#");
  lines.push("# HOW TO USE:");
  lines.push("#   1. Create a NEW empty (or nearly empty) repository on GitHub.");
  lines.push("#      A repo with pre-existing commits on these dates will make");
  lines.push("#      the pattern impossible or blobby — prefer a fresh one.");
  lines.push("#   2. The commit email must match one of the emails verified on");
  lines.push("#      your GitHub account, otherwise contributions will not count.");
  ...
  lines.push("else");
  lines.push('  echo "==> No repository URL provided, using current directory."');
  lines.push('  echo "    Make sure \'git init\' has been run and a remote is configured."');
  lines.push("fi");
  lines.push("");
  ...
  for (const [i, entry] of plan.entries()) {
    ...
    lines.push(`# (${i + 1}/${plan.length}) ${entry.date} — ${entry.count} commit${entry.count > 1 ? "s" : ""}`);
    for (let c = 1; c <= entry.count; c++) {
      const verb = ["Touch", "Warm", "Tune", "Restyle", "Refresh", "Sharpen", "Polish"][
        i % 7
      ];
      lines.push(
  ...
  lines.push("git push -u origin \"$BRANCH\" 2>/dev/null || git push origin \"$BRANCH\"");
  lines.push("");
  lines.push('echo ""');
