...
execFileSync(process.execPath, [
  "node_modules/typescript/bin/tsc", "lib/grid.ts", "lib/fonts.ts",
  "lib/storage.ts", "data/templates.ts", "--outDir", output,
...
test("each year has exactly its real days, aligned Sunday–Saturday, including leap years", () => {
  for (let year = 2000; year <= 2100; year++) {
    ...
    for (let x = 0; x < columns; x++) for (let y = 0; y < 7; y++) {
...
test("normalization and fill preserve all calendar dates, including future dates", () => {
  ...
  const full = calendar.emptyGrid(calendar.calendarColumns(year)).map(row => row.map(() => 4));
  const cleaned = calendar.normalizeGrid(full, year);
  cleaned.forEach((row, y) => row.forEach((value, x) =>
    assert.equal(value, calendar.canPaint(y, x, year) ? 4 : 0)));
  const pastYear = 2024;
  const blank = calendar.emptyGrid(calendar.calendarColumns(pastYear));
  const result = calendar.floodFill(blank, 1, 0, 4, (x, y) => calendar.canPaint(y, x, pastYear));
...
test("all templates are rectangular and fit without clipping or changing active pixels", () => {
  for (const template of TEMPLATES) {
    ...
    assert.equal(placed.flat().filter(Boolean).length, pattern.flat().filter(Boolean).length);
    placed.forEach((row, y) => row.forEach((v, x) => {
...
test("text width matches output, including accents, unknown symbols and empty text", () => {
  for (const text of ["", "CODE", "HIRE ME!", "été", "A🙂B", "你好"]) {
    ...
    assert.ok(grid.every(row => row.length === width), text);
  }
  assert.equal(textToGrid("CODE", "5x7").width, 23);
  assert.deepEqual(textToGrid("été", "5x7"), textToGrid("ETE", "5x7"));
  assert.equal(calendar.placePattern(textToGrid("X".repeat(100), "5x7").grid, 2024), null);
  assert.ok(calendar.placePattern(textToGrid("", "5x7").grid, 2024).flat().every(v => v === 0));
});
...
test("future drawing survives normalization, sharing and the daily commit plan", () => {
  ...
  assert.deepEqual(decodeGrid(encodeGrid(preserved), preserved[0].length), grid);
  const plan = calendar.buildCommitPlan(preserved, year, calendar.DEFAULT_THRESHOLDS, "", true);
  assert.equal(plan.length, 1);
  assert.equal(plan[0].count, 5);
  assert.equal(plan[0].date, calendar.dateToKey(calendar.cellToDate(3, 10, year)));
  assert.equal(calendar.buildCommitPlan(preserved, year, calendar.DEFAULT_THRESHOLDS, "", false).length, 0);
});
